...
public class TemaController {
	...
	public ResponseEntity<List<Tema>> getByPalavraChave(@PathVariable String palavraChave) {
		return ResponseEntity.ok(temaRepository.findByPalavraChaveContainingIgnoreCase(palavraChave));
	}
	...
	public ResponseEntity<Tema> putTema(@RequestBody Tema tema) {
		return temaRepository.findById(tema.getId()).map(resposta -> ResponseEntity.ok(temaRepository.save(tema)))
				.orElse(ResponseEntity.notFound().build());
	}
